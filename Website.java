 ...
 * payForWine method.
 * 
 * There are 5 mutator methods first of which buyerLogin allow existing browser to 
 * login to the website and ammend the hits field within website and isBuyer fields
 * in the browser object. The following method buyerLogout allows browser to log out ONLY
 * if its logged in already. The next mutator method is becomeBuyer. The fucntion of this 
 * method is to determine if the browser is old enough to register and if he is a new user. 
 * Once the age and acount check are complieted, the method calls the previous method 
 * buyerLogin. Next we have the checkout() mutator method which contain a nested
 ...
 * determine if the browser is eligible for discount. The following mutator method 
 * setSalesTotal amend the salesTotal field in the website class when called from
 * the payForWine method in the browser object. Last but not least, CheckHitDescount 
...
public class Website
{
     ...
     * amend the boolean isBuyer and String website to false and null respectively. Also, print
     * a thank you message in the terminal window.
     */
    ...
    public void checkout(Browser browser)
    {
        ...
        else
        {
            if (checkHitDiscount())
            {
                ...
                salesTotal = salesTotal + wineCase.getPrice() * 0.9;
            }
            else
            {
                System.out.print('\u000C');
